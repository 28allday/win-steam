...
udevadm control --reload 2>/dev/null || true  # no udev daemon under WSL2
...
mount "$HOMEPART" "$HOMEMNT" 2>/dev/null || { log "home mount failed — kernel lacks ext4 casefold support (WSL2); clearing the unused casefold flag"; e2fsck -fy "$HOMEPART" >/dev/null 2>&1 || true; debugfs -w -R "feature -casefold" "$HOMEPART" >/dev/null 2>&1; e2fsck -fy "$HOMEPART" >/dev/null 2>&1 || true; mount "$HOMEPART" "$HOMEMNT"; }
...
PACOPTS="--noconfirm --needed --disable-download-timeout"
